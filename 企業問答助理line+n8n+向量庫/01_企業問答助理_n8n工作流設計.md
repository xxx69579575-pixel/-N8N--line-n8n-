...
> **實作狀態：✅ 已完整部署**
> 三條 n8n Workflow 均已完成並通過測試：問答流程 / 文件匯入流程 / 資料庫自動備份
...
## 十三、實作完成紀錄（2026-03-26）

### 已部署的三條 n8n Workflow

| Workflow 檔案 | 功能 | 觸發方式 | 狀態 |
|---|---|---|---|
| `workflows/qa_workflow.json` | LINE 問答（RAG） | LINE Webhook | ✅ 運行中 |
| `workflows/ingest_workflow_v2.json` | 文件自動匯入 | 每小時排程 | ✅ 運行中 |
| `workflows/backup_workflow.json` | 資料庫自動備份 | 每天凌晨 2:00 | ✅ 運行中 |

### Python API Server（scripts/api_server.py，port 8765）

所有 n8n Workflow 均透過 HTTP Request 節點呼叫 API Server，取代 n8n 2.12+ 已封鎖的 executeCommand 節點。

| 端點 | 功能 |
|---|---|
| `GET /health` | 健康檢查 |
| `GET /list-inbox` | 掃描文件收件匣（含子資料夾分類） |
| `GET /files/<name>` | 下載原始檔案 |
| `POST /line-verify` | LINE 簽章驗證 |
| `POST /vector-search` | pgvector 相似度搜尋 |
| `POST /prompt-builder` | 組裝 RAG Prompt |
| `POST /search-files` | 按關鍵字搜尋檔案 |
| `POST /ingest-file` | 單檔完整匯入 pipeline |
| `POST /backup-db` | 觸發 PostgreSQL 備份 |

### 文件匯入流程（實際架構）

```
文件資料夾（子資料夾名稱 = department 分類）
    ↓ 每小時自動掃描
POST /list-inbox → 取得待處理檔案清單
    ↓ 逐檔處理
POST /ingest-file：
  extract_text.py → chunk_text.py → embed_chunks.py → write_to_db.py
    ↓ 完成後
移至 processed/（成功）或 error/（失敗）
```

### 自動備份

- 備份路徑：`D:\智能助理資料庫自動備份`
- 保留份數：最近 7 份（.sql 格式，超過自動刪除）
- 還原指令：`docker exec -i pg_container psql -U testuser -d vectordb < backup.sql`

### 系統啟動順序

1. Docker（PostgreSQL + n8n）：`docker compose up -d`
2. Ollama：開機自動啟動
3. Python API Server：`start_api_server.bat`（開機自動啟動）
4. ngrok：`start_ngrok.bat`（開機自動啟動）
