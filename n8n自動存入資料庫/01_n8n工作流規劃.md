...
> **實作狀態：✅ 已完整部署**
> `workflows/ingest_workflow_v2.json` 已完成並通過測試，每小時自動掃描文件資料夾。
...
**子資料夾規則**：第一層子資料夾名稱自動成為 `department` 標籤，深層子資料夾不影響（仍以第一層為準）。根目錄直接放的檔案 department = `general`。

> 舊設計的 `processing` 暫存資料夾已取消，改為 api_server 直接在原路徑處理後搬檔。

## 六、n8n 流程節點設計

### 1. Trigger：Schedule Trigger

排程掃描，**每 1 小時**執行一次（可依需求調整）。另附 Manual Trigger 供隨時手動觸發測試。

### 2. 讀取檔案清單

呼叫 `GET /list-inbox`（api_server 端點），掃描 `INGEST_INBOX_DIR` 及其子資料夾，回傳待處理檔案清單。**子資料夾名稱自動對應 `department` 分類**（例如 `inbox/建設公司/` → department = "建設公司"）。

> ⚠️ n8n 2.12+ 已封鎖 Execute Command 節點，本工作流改用 HTTP Request 節點呼叫 api_server。
...
## 十二、實作完成紀錄（2026-03-26）

### Workflow 實際架構（ingest_workflow_v2.json）

```
Schedule（每1小時）＋ Manual Trigger
         ↓
GET /list-inbox     ← 掃描 INGEST_INBOX_DIR（含子資料夾分類）
         ↓
IF count > 0？
   ↓ 是
Split：逐檔處理
         ↓
POST /ingest-file（timeout 300s）
  內部執行：
    extract_text.py  → 抽文字（PDF/Word/Excel/圖片）
    chunk_text.py    → 切片（800字/片，overlap 150）
    embed_chunks.py  → Ollama bge-m3 Embedding（1024維）
    write_to_db.py   → 寫入 documents / chunks / permissions
         ↓
IF success？
  ✅ 移至 processed/ + Log 成功
  ❌ 移至 error/    + Log 失敗
...
| 項目 | 結果 |
|---|---|
| PDF 文字抽取 + 向量化 | ✅ 楊富段投報分析.pdf → 4 個切片 |
| SHA-256 去重 | ✅ 重複檔案自動略過 |
| 子資料夾自動分類 | ✅ 建設公司/ → department="建設公司" |
| 成功後移至 processed/ | ✅ |
| 失敗後移至 error/ | ✅ |
| n8n 2.12+ 相容 | ✅ 全 HTTP Request 節點，無 executeCommand |
